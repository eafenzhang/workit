...
import { WrenchIcon, UploadIcon, TrashIcon } from 'lucide-react';
...
const PLUGIN_TEMPLATE = [
  { name: 'code-reviewer', description: 'AI 代码审查', source: 'marketplace', config: { version: '1.0.0' } },
...
export default function PluginsTab({ hideToolbar }: { hideToolbar?: boolean }) {
  ...
  const fetchItems = () => {
    apiFetch(API.plugins)
      .then((r: any) => r.json())
      .then((data: PluginItem[]) => setItems(Array.isArray(data) ? data : []))
      .catch(() => {});
  };
  ...
  const deleteItem = (id: string) => {
    ...
    apiFetch(API.pluginsById(id), { method: 'DELETE' }).then(() => {
      fetchItems();
      toast.success('已删除');
    });
  };
    ...
    <div className="flex flex-col">
      ...
      {items.length === 0 && (
        ...
        >
          ...
          <p className="mt-4 text-wiki-text2 text-sm">暂无 Claude Code 插件</p>
          <p className="mt-1 text-wiki-text3 text-xs">点击「一键导入」快速添加插件</p>
        </div>
      )}
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  background: item.enabled ? 'rgba(245,158,11,0.12)' : 'var(--wiki-surface2)',
                }}
              >
                <WrenchIcon
                  size={18}
                  style={{ color: item.enabled ? '#f59e0b' : 'var(--wiki-text3)' }}
                />
              </div>
