...
import { TerminalIcon, UploadIcon, TrashIcon } from 'lucide-react';
...
const CLI_TEMPLATE = [
  { name: "gh", description: "GitHub CLI", source: "marketplace", config: {} },
  { name: "kubectl", description: "Kubernetes CLI", source: "built-in", config: {} },
];
...
export default function CliToolsTab({ hideToolbar }: { hideToolbar?: boolean }) {
  ...
  const fetchItems = () => {
    apiFetch(API.cliTools).then(r => r.json()).then(data => setItems(Array.isArray(data) ? data : [])).catch(() => {});
  };
  ...
  const deleteItem = (id: string) => {
    ...
    apiFetch(API.cliToolsById(id), { method: 'DELETE' }).then(() => { fetchItems(); toast.success('已删除'); });
  };
    ...
    <div className="flex flex-col">
      ...
      {items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 mx-8 rounded-lg" style={{ background: 'var(--wiki-surface)', border: '1px solid var(--wiki-border)' }}>
          ...
          <p className="mt-4 text-wiki-text2 text-sm">暂无 CLI 工具</p>
          <p className="mt-1 text-wiki-text3 text-xs">点击「一键导入」快速添加 CLI 工具</p>
        </div>
      )}
      <div className="flex flex-col gap-3 px-8 pb-8">
        {items.map(item => (
          <div key={item.id} className="rounded-lg p-4" style={{ background: 'var(--wiki-surface)', border: '1px solid var(--wiki-border)' }}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: 'var(--wiki-surface2)', color: 'var(--wiki-text3)' }}>
                    {sourceLabels[item.source] || item.source}
                  </span>
                </div>
                {item.description && <div className="text-xs text-wiki-text3 mt-0.5 line-clamp-1">{item.description}</div>}
              </div>
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <button onClick={() => deleteItem(item.id)} className="p-1.5 rounded hover:bg-wiki-surface2">
      ...
      {!hideToolbar && (
      <ImportModal open={showImport} onClose={() => setShowImport(false)} onImported={fetchItems}
        apiPrefix={API.cliTools} title="导入 CLI 工具" template={CLI_TEMPLATE} />
      )}
    </div>
  );
}
