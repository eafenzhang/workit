...
const statusConfig: Record<string, { color: string; bg: string; icon: typeof CheckCircleIcon }> = {
  ...
  '实现中': { color: `#8b5cf6`, bg: `rgba(139,92,246,0.12)`, icon: ClockIcon },
  '测试中': { color: `#06b6d4`, bg: `rgba(6,182,212,0.12)`, icon: ClockIcon },
    ...
    <div data-cmp="Requirements" className="flex h-full overflow-hidden">
      ...
      <div className="flex flex-col flex-1 p-8 overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: `var(--wiki-surface)`, border: `1px solid rgba(99,112,196,0.2)` }}>
            {statuses.map((s) => (
              <button key={s} onClick={() => setFilterStatus(s)}
                className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
                style={{ background: filterStatus === s ? `linear-gradient(135deg, #6366f1, #8b5cf6)` : `transparent`, color: filterStatus === s ? `#fff` : `var(--wiki-text2)` }}>
                {s}
              </button>
            ))}
          </div>
          <button onClick={() => setShowFilter(!showFilter)}
            className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm" style={{ background: `var(--wiki-surface)`, border: `1px solid rgba(99,112,196,0.2)`, color: `var(--wiki-text2)` }}>
            <FilterIcon size={14} />
            <span>筛选</span>
        ...
        {/* Advanced Filters */}
        {showFilter && (
          <div className="mb-4 p-4 rounded-xl" style={{ background: `var(--wiki-surface)`, border: `1px solid rgba(99,112,196,0.2)` }}>
            <div className="flex flex-wrap gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-wiki-text3">模块</label>
                <select className="px-3 py-2 rounded-lg text-sm text-wiki-text outline-none" style={{ background: `rgba(99,112,196,0.1)`, border: `1px solid rgba(99,112,196,0.2)` }}
              ...
              <div className="flex flex-col gap-1">
                ...
                <input className="px-3 py-2 rounded-lg text-sm text-wiki-text outline-none w-28" style={{ background: `rgba(99,112,196,0.1)`, border: `1px solid rgba(99,112,196,0.2)` }}
              ...
              <div className="flex flex-col gap-1">
                ...
                <input type="date" className="px-3 py-2 rounded-lg text-sm text-wiki-text outline-none" style={{ background: `rgba(99,112,196,0.1)`, border: `1px solid rgba(99,112,196,0.2)` }}
        ...
        <div className="flex gap-3 mb-4">
          {[
            { label: `全部`, count: requirements.length, color: `#6366f1` },
            { label: `待评审`, count: requirements.filter(r => r.status === '待评审').length, color: `#f59e0b` },
            { label: `设计中`, count: requirements.filter(r => r.status === '设计中').length, color: `#6366f1` },
            { label: `实现中`, count: requirements.filter(r => r.status === '实现中').length, color: `#8b5cf6` },
            { label: `测试中`, count: requirements.filter(r => r.status === '测试中').length, color: `#06b6d4` },
            { label: `已完成`, count: requirements.filter(r => r.status === '已完成').length, color: `#10b981` },
          ].map((stat) => (
            <div key={stat.label} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: `rgba(99,112,196,0.06)` }}>
          ...
          {requirements.map((req) => {
            ...
            return (
              <div key={req.id}
                ...
                style={{
                  background: isSelected ? `linear-gradient(135deg, rgba(99,102,241,0.12), rgba(139,92,246,0.08))` : 'var(--wiki-surface)',
                  border: isSelected ? `1px solid rgba(99,102,241,0.4)` : `1px solid rgba(99,112,196,0.2)`,
                }}>
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3 flex-1">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `rgba(139,92,246,0.12)` }}>
                      <SparklesIcon size={14} style={{ color: `#8b5cf6` }} />
                    </div>
                ...
                <div className="flex items-center justify-between mt-2 flex-wrap gap-2">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-md" style={{ background: `rgba(99,102,241,0.1)`, color: `var(--wiki-text2)` }}>{req.module}</div>
                  </div>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {req.aiTags.map((tag) => (
                      <span key={tag} className="text-xs px-2 py-0.5 rounded-md" style={{ background: `rgba(139,92,246,0.15)`, color: `#8b5cf6` }}>{tag}</span>
                    ))}
                  </div>
                </div>
                {req.images?.length > 0 && (
                  <div className="flex gap-2 mt-2">
                    {req.images.map((img, i) => (
                      <img key={i} src={img} className="w-12 h-12 rounded object-cover" style={{ border: `1px solid rgba(99,112,196,0.2)` }} />
                    ))}
                  </div>
                )}
              </div>
      ...
      {selectedReq && !editingReq && (
        <div className="flex-1 flex flex-col overflow-hidden" style={{ borderLeft: '1px solid var(--wiki-border)', background: 'var(--wiki-surface)' }}>
          ...
          <div className="flex items-center gap-3 p-4" style={{ borderBottom: '1px solid var(--wiki-border)' }}>
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-2 mt-1.5">
                <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366f1' }}>{selectedReq.module}</span>
          ...
          {selectedReq.aiSummary || selectedReq.aiTags.length > 0 || selectedReq.imageDescriptions.length > 0 ? (
            <div className="mx-6 mt-4 p-4 rounded-xl border" style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.15), rgba(99,102,241,0.1))', border: '1px solid rgba(139,92,246,0.4)' }}>
              <div className="flex items-center gap-2 mb-3">
                <SparklesIcon size={14} style={{ color: '#8b5cf6' }} />
                <span className="text-xs font-bold" style={{ color: '#8b5cf6' }}>AI 分析结果</span>
              </div>
              ...
              {selectedReq.aiTags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {selectedReq.aiTags.map((tag) => (<span key={tag} className="text-xs px-2 py-0.5 rounded-lg" style={{ background: 'rgba(139,92,246,0.15)', color: '#8b5cf6' }}>{tag}</span>))}
                </div>
              )}
            </div>
          ) : (
            <div className="mx-6 mt-4">
              <button onClick={() => handleAnalyze(selectedReq)} disabled={analyzing} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm" style={{ background: analyzing ? 'rgba(99,102,241,0.5)' : 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff' }}>
          ...
          {/* Workflow */}
          <div className="mx-6 mt-4 p-4 rounded-xl" style={{ background: 'rgba(99,112,196,0.06)', border: '1px solid rgba(99,112,196,0.15)' }}>
                ...
                return (
                  <div key={step} className="flex items-center" style={{ flex: i < 4 ? 1 : 'none' }}>
                    <div className="flex flex-col items-center">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium" style={{ background: isDone ? '#10b981' : isCurrent ? '#6366f1' : 'rgba(99,112,196,0.15)', color: isDone || isCurrent ? '#fff' : 'var(--wiki-text3)' }}>
                        {isDone ? '✓' : i + 1}
                      </div>
                      <div className="text-xs mt-1 text-center" style={{ color: isCurrent ? '#6366f1' : 'var(--wiki-text3)', fontSize: '10px' }}>{step}</div>
                    </div>
                    {i < 4 && <div className="flex-1 h-0.5 mx-1" style={{ background: isDone ? '#10b981' : 'rgba(99,112,196,0.2)' }} />}
                  </div>
                );
              })}
            </div>
            {selectedReq.status !== '已完成' && (
              <button onClick={() => { const statusOrder = ['待评审', '设计中', '实现中', '测试中', '已完成']; const nextIdx = statusOrder.indexOf(selectedReq.status) + 1; if (nextIdx < statusOrder.length) { fetch(`/api/requirements/${selectedReq.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: selectedReq.title, desc: selectedReq.desc, module: selectedReq.module, priority: selectedReq.priority, status: statusOrder[nextIdx], assignee: selectedReq.assignee, workflow_handler: selectedReq.assignee, images: selectedReq.images }) }).then(() => { fetchRequirements(); toast.success(`已推进到「${statusOrder[nextIdx]}」`); }); } }} className="w-full py-2 rounded-xl text-xs font-medium text-white" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
                推进到下一步
              </button>
            )}
            {selectedReq.workflowHistory?.length > 0 && (
              <div className="mt-4 pt-3" style={{ borderTop: '1px solid rgba(99,112,196,0.12)' }}>
                <div className="text-xs text-wiki-text3 mb-2">流转记录</div>
                {selectedReq.workflowHistory.map((h, idx) => (
                  <div key={idx} className="flex items-center justify-between py-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366f1' }}>{h.from}</span>
                      <span className="text-xs text-wiki-text3">→</span>
                      <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'rgba(16,185,129,0.1)', color: '#10b981' }}>{h.to}</span>
                    </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
            <div className="max-w-3xl mx-auto flex flex-col gap-5">
              {/* Description */}
              <div className="p-4 rounded-xl" style={{ background: 'rgba(99,112,196,0.08)', border: '1px solid rgba(99,112,196,0.15)' }}>
                ...
                {selectedReq.images?.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {selectedReq.images.map((img, i) => (
                      <img key={i} src={img} className="rounded object-cover cursor-pointer" style={{ border: '1px solid rgba(99,112,196,0.2)', width: '80px', height: '80px' }} onClick={() => window.open(img, '_blank')} />
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between py-2.5" style={{ borderBottom: '1px solid rgba(99,112,196,0.12)' }}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedReq.tags.map((tag) => (
                      <span key={tag} className="text-xs px-2.5 py-1 rounded-lg" style={{ background: 'rgba(99,102,241,0.15)', color: 'var(--wiki-text2)' }}>{tag}</span>
                    ))}
                  </div>
                </div>
              )}
        ...
        <div className="flex-1 flex flex-col" style={{ borderLeft: '1px solid var(--wiki-border)' }}>
          <div className="flex items-center gap-3 p-4" style={{ borderBottom: '1px solid var(--wiki-border)', background: 'var(--wiki-surface)' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={editingReq ? handleUpdate : handleCreate} className="px-4 py-2 rounded-xl text-sm font-medium text-white" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-thin" style={{ background: 'var(--wiki-bg)' }}>
            <div className="max-w-3xl mx-auto flex flex-col gap-5">
              ...
              <div>
                ...
                <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm" style={{ background: 'rgba(99,112,196,0.1)', color: 'var(--wiki-text2)', border: '1px solid rgba(99,112,196,0.2)' }}>
