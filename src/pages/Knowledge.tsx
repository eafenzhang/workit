...
export default function Knowledge() {
  ...
  const handleDelete = (id: number) => {
    ...
    fetch(`/api/documents/${id}`, { method: 'DELETE' }).then(() => { fetchDocs(); toast.success('已删除'); });
  };
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Left Sidebar */}
      <div className="flex flex-col w-[220px] min-w-[220px] p-5 overflow-hidden" style={{ borderRight: '1px solid var(--wiki-border)', background: 'var(--wiki-surface)' }}>
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={handleDeleteCategory} disabled={activeCategory === 'all'} className="w-6 h-6 rounded-lg flex items-center justify-center transition-colors disabled:opacity-30 disabled:cursor-not-allowed hover:bg-wiki-surface2" title="删除分类">
            ...
            if (viewMode === 'grid') {
              ...
              return (
                <div key={doc.id} onClick={() => { fetch(`/api/documents/${doc.id}`).then(r => r.json()).then(setShowDoc); }} className="flex items-center gap-4 px-4 py-3 rounded-xl cursor-pointer transition-all duration-200 hover:border-indigo-500/30" style={{ background: 'var(--wiki-surface)', border: '1px solid var(--wiki-border)' }}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); handleDelete(doc.id); }} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(239,68,68,0.12)' }}><TrashIcon size={12} style={{ color: '#ef4444' }} /></button>
                </div>
              );
            }
          })}
      ...
      {showEdit && (
        <div className="flex-1 flex flex-col" style={{ borderLeft: '1px solid var(--wiki-border)' }}>
          ...
          <div className="flex items-center gap-3 p-4" style={{ borderBottom: '1px solid var(--wiki-border)', background: 'var(--wiki-surface)' }}>
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="px-3 py-1.5 rounded-lg text-sm text-wiki-text outline-none"
          ...
          <div className="flex items-center gap-1 px-4 py-2" style={{ borderBottom: '1px solid var(--wiki-border)', background: 'var(--wiki-surface)' }}>
            <button onClick={() => editor?.chain().focus().toggleBold().run()} className={`p-2 rounded-lg ${editor?.isActive('bold') ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="粗体"><BoldIcon size={14} style={{ color: 'var(--wiki-text)' }} /></button>
            <button onClick={() => editor?.chain().focus().toggleItalic().run()} className={`p-2 rounded-lg ${editor?.isActive('italic') ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="斜体"><ItalicIcon size={14} style={{ color: 'var(--wiki-text)' }} /></button>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--wiki-border)' }} />
            <button onClick={() => editor?.chain().focus().toggleHeading({ level: 1 }).run()} className={`p-2 rounded-lg ${editor?.isActive('heading', { level: 1 }) ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="标题1"><span className="text-xs font-bold" style={{ color: 'var(--wiki-text)' }}>H1</span></button>
            <button onClick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()} className={`p-2 rounded-lg ${editor?.isActive('heading', { level: 2 }) ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="标题2"><span className="text-xs font-bold" style={{ color: 'var(--wiki-text)' }}>H2</span></button>
            <button onClick={() => editor?.chain().focus().toggleHeading({ level: 3 }).run()} className={`p-2 rounded-lg ${editor?.isActive('heading', { level: 3 }) ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="标题3"><span className="text-xs font-bold" style={{ color: 'var(--wiki-text)' }}>H3</span></button>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--wiki-border)' }} />
            <button onClick={() => editor?.chain().focus().toggleBulletList().run()} className={`p-2 rounded-lg ${editor?.isActive('bulletList') ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="列表"><ListIcon2 size={14} style={{ color: 'var(--wiki-text)' }} /></button>
            <button onClick={() => editor?.chain().focus().toggleOrderedList().run()} className={`p-2 rounded-lg ${editor?.isActive('orderedList') ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="编号列表"><ClockIcon size={14} style={{ color: 'var(--wiki-text)' }} /></button>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--wiki-border)' }} />
            <button onClick={() => editor?.chain().focus().toggleBlockquote().run()} className={`p-2 rounded-lg ${editor?.isActive('blockquote') ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="引用"><span className="text-xs" style={{ color: 'var(--wiki-text)' }}>"</span></button>
            <button onClick={() => editor?.chain().focus().toggleCodeBlock().run()} className={`p-2 rounded-lg ${editor?.isActive('codeBlock') ? 'bg-indigo-100' : 'hover:bg-wiki-surface2'}`} title="代码块"><span className="text-xs font-mono" style={{ color: 'var(--wiki-text)' }}>&lt;&gt;</span></button>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--wiki-border)' }} />
            <button onClick={() => imageInputRef.current?.click()} className="p-2 rounded-lg hover:bg-wiki-surface2" title="上传图片"><ImagePlusIcon size={14} style={{ color: 'var(--wiki-text)' }} /></button>
            <button onClick={() => { const el = linkInputRef.current; if (el) { el.style.display = el.style.display === 'none' ? 'flex' : 'none'; el.style.alignItems = 'center'; el.style.gap = '4px'; } }} className="p-2 rounded-lg hover:bg-wiki-surface2" title="插入链接"><Link2Icon size={14} style={{ color: 'var(--wiki-text)' }} /></button>
            <div className="w-px h-5 mx-1" style={{ background: 'var(--wiki-border)' }} />
            <button onClick={() => editor?.chain().focus().undo().run()} className="p-2 rounded-lg hover:bg-wiki-surface2" title="撤销"><Undo2Icon size={13} style={{ color: 'var(--wiki-text)' }} /></button>
            <button onClick={() => editor?.chain().focus().redo().run()} className="p-2 rounded-lg hover:bg-wiki-surface2" title="重做"><Redo2Icon size={13} style={{ color: 'var(--wiki-text)' }} /></button>
            {/* Hidden image input */}
          ...
          <div className="w-[400px] rounded-2xl p-6" style={{ background: 'var(--wiki-surface)', border: '1px solid var(--wiki-border)' }}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input value={showCategoryEdit.name || ''} onChange={(e) => setShowCategoryEdit(prev => ({ ...prev!, name: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl text-sm text-wiki-text outline-none" style={{ background: 'var(--wiki-surface2)', border: '1px solid var(--wiki-border)' }} />
              </div>
              <div>
