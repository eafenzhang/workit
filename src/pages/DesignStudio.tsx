...
function DesignEditor({docId,onClose}:{docId:number;onClose:()=>void}) {
  ...
  const aiGenFn=async()=>{if(!aiIn.trim())return;setChatMsgs(prev=>[...prev,{role:'user',content:aiIn}]);setAiGen(true);const ctx=nodes.length>0?`\n当前节点:${JSON.stringify(nodes)}`:'';
    const sp=`你是UI设计师。输出纯JSON数组[DesignNode]。节点:frame(容器,layout:vertical|horizontal+gap+padding+children),rect,text,ellipse,image。画布${vp}x768。每个节点必填type,x,y,width,height。text需content,fontSize。颜色:#6366f1/#10b981/#f59e0b/#ef4444/#333/#fff/#f8f9fa。圆角8-16px。${ctx}`;
    try{const api=(window as any).electronAPI;const r=await api?.chatSend?.({messages:[{role:'user',content:aiIn}],systemPrompt:sp,toolsEnabled:false});const reply=(r?.content||'').replace(/```[a-z]*\n?/gi,'').replace(/`/g,'').trim();const m=reply.match(/\[[\s\S]*\]/);if(!m)throw new Error('格式异常');const parsed=JSON.parse(m[0]);const fresh=(parsed||[]).map((el:any,i:number)=>({...el,id:'ai_'+Date.now()+'_'+i}));setNodes(aiIn.includes('修改')?[...nodes,...fresh]:fresh);setChatMsgs(prev=>[...prev,{role:'assistant',content:`已生成${fresh.length}个节点`}]);toast.success(`已${aiIn.includes('修改')?'添加':'生成'}${fresh.length}个节点`);setAiIn('')}catch(e:any){setChatMsgs(prev=>[...prev,{role:'assistant',content:'生成失败:'+(e.message||'')}]);toast.error('生成失败')}finally{setAiGen(false)}};
...
function DesignCreator({onClose}:{onClose:()=>void}) {
  ...
  const genFn=async()=>{if(!aiIn.trim())return;setChatMsgs(prev=>[...prev,{role:'user',content:aiIn}]);setGen(true);
    const sp=`你是UI设计师。输出纯JSON数组[DesignNode]。类型:frame(容器,layout:vertical|horizontal+gap+padding+children),rect,text,ellipse,image。画布${vp}x768。颜色:#6366f1/#10b981/#f59e0b/#ef4444/#333/#fff/#f8f9fa。圆角8-16px。每个节点必填type,x,y,width,height。text需content,fontSize。生成完整页面15-30个节点。`;
    try{const api=(window as any).electronAPI;const r=await api?.chatSend?.({messages:[{role:'user',content:aiIn}],systemPrompt:sp,toolsEnabled:false});const reply=(r?.content||'').replace(/```[a-z]*\n?/gi,'').replace(/`/g,'').trim();const m=reply.match(/\[[\s\S]*\]/);if(!m)throw new Error('格式异常');const parsed=JSON.parse(m[0]);const fresh=(parsed||[]).map((el:any,i:number)=>({...el,id:'ai_'+Date.now()+'_'+i}));setNodes(aiIn.includes('修改')?[...nodes,...fresh]:fresh);setTitle(title||aiIn.substring(0,30));setChatMsgs(prev=>[...prev,{role:'assistant',content:`已生成${fresh.length}个节点`}]);toast.success(`已生成${fresh.length}个节点`);setAiIn('')}catch(e:any){setChatMsgs(prev=>[...prev,{role:'assistant',content:'失败:'+(e.message||'')}]);toast.error('失败')}finally{setGen(false)}};
