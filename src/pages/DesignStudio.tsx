...
function DesignEditor({docId,onClose}:{docId:number;onClose:()=>void}) {
  ...
  const [rightTab,setRightTab]=useState<'style'|'code'>('style');
  ...
  const aiGenFn=async()=>{if(!aiIn.trim())return;setChatMsgs(prev=>[...prev,{role:'user',content:aiIn}]);setAiGen(true);const ctx=nodes.length>0?`\n当前节点:${JSON.stringify(nodes)}`:'';
    const sp=`你是资深UI/UX设计师。你必须严格只输出一个纯JSON数组，不要markdown代码块、不要解释文字。画布${vp}×768px。元素(DesignNode)必含type,x,y,width,height。type:frame(容器,可嵌套children,支持layout/gap/padding),rect,text(需content+fontSize),ellipse,image。颜色:#6366f1/#10b981/#f59e0b/#ef4444/#333/#fff/#f8f9fa/#e5e7eb。圆角8-16px。仅输出数组，如:[{"type":"rect","x":40,"y":40,"width":200,"height":100,"fill":"#6366f1","cornerRadius":12}]。${ctx}`;
    try{const api=(window as any).electronAPI;if(!api?.chatSend)throw new Error('IPC不可用，请重启');const r=await api.chatSend({messages:[{role:'user',content:aiIn}],systemPrompt:sp,toolsEnabled:false});if(!r)throw new Error('无响应');if(r.error)throw new Error(r.error);let raw=(r.content||'').trim();if(!raw)throw new Error('AI返回空内容');let reply=raw.replace(/```[a-z]*\n?/gi,'').replace(/`/g,'').trim();const m=reply.match(/\[[\s\S]*\]/);if(!m)throw new Error('AI未返回JSON数组，开头: '+raw.substring(0,80));let parsed;try{parsed=JSON.parse(m[0])}catch(pe){throw new Error('JSON解析失败: '+pe.message)}if(!Array.isArray(parsed)||parsed.length===0)throw new Error('返回的不是有效数组');const fresh=parsed.map((el,i:any)=>({...el,id:'ai_'+Date.now()+'_'+i}));setNodes(aiIn.includes('修改')?[...nodes,...fresh]:fresh);setChatMsgs(prev=>[...prev,{role:'assistant',content:'已生成'+fresh.length+'个节点'}]);toast.success('已'+(aiIn.includes('修改')?'添加':'生成')+fresh.length+'个节点');setAiIn('')}catch(e){const em=(e as any).message||String(e);setChatMsgs(prev=>[...prev,{role:'assistant',content:'生成失败: '+em}]);toast.error('生成失败: '+em)}finally{setAiGen(false)}};
  ...
  return(<div className="flex flex-col h-full">
    ...
    <div className="flex-1 flex overflow-hidden">
      {/* Zone 1: Left sidebar — pages + layers */}
      <div className="w-44 flex-shrink-0 flex flex-col" style={{borderRight:'1px solid var(--wiki-border)',background:'var(--wiki-surface)'}}>
        <div className="flex border-b" style={{borderColor:'var(--wiki-border)'}}>
          <button onClick={()=>setSideTab('layers')} className="flex-1 py-2 text-[11px] font-medium" style={{color:sideTab==='layers'?'var(--wiki-text)':'var(--wiki-text3)',borderBottom:sideTab==='layers'?'2px solid var(--wiki-text)':'2px solid transparent'}}>图层</button>
          <button onClick={()=>setSideTab('pages')} className="flex-1 py-2 text-[11px] font-medium" style={{color:sideTab==='pages'?'var(--wiki-text)':'var(--wiki-text3)',borderBottom:sideTab==='pages'?'2px solid var(--wiki-text)':'2px solid transparent'}}>画板</button>
        </div>
        {sideTab==='layers'?(
          <div className="flex-1 overflow-y-auto scrollbar-thin">
            {layers.length===0?<div className="text-[10px] text-wiki-text3 p-3 text-center">暂无图层</div>:
            layers.map(({node,depth})=>(
              <div key={node.id} onClick={()=>setSel(node.id)}
                className="flex items-center gap-1.5 px-2 py-1 cursor-pointer text-[11px] hover:bg-wiki-surface2"
          ...
          <div className="flex-1 p-2 space-y-1">
            <div className="text-[10px] text-wiki-text3 px-1 py-0.5">当前画板</div>
            <div className="text-xs px-2 py-1 rounded" style={{background:'var(--wiki-surface2)',color:'var(--wiki-text)'}}>{title||'未命名'}</div>
          </div>)}
      </div>
      ...
      <div className="w-52 flex-shrink-0 flex flex-col" style={{borderLeft:'1px solid var(--wiki-border)',background:'var(--wiki-surface)'}}>
        <div className="flex border-b" style={{borderColor:'var(--wiki-border)'}}>
          <button onClick={()=>setRightTab('style')} className="flex-1 py-2 text-[11px] font-medium" style={{color:rightTab==='style'?'var(--wiki-text)':'var(--wiki-text3)',borderBottom:rightTab==='style'?'2px solid var(--wiki-text)':'2px solid transparent'}}>样式</button>
          <button onClick={()=>setRightTab('code')} className="flex-1 py-2 text-[11px] font-medium" style={{color:rightTab==='code'?'var(--wiki-text)':'var(--wiki-text3)',borderBottom:rightTab==='code'?'2px solid var(--wiki-text)':'2px solid transparent'}}>代码</button>
        </div>
        {rightTab==='style'?(
          <div className="flex-1 overflow-y-auto scrollbar-thin">
            {selNode?<div className="p-3 space-y-2.5">
              <div><div className="text-[10px] text-wiki-text3 mb-1">类型</div><div className="text-xs text-wiki-text">{selNode.type}{selNode.role?` (${selNode.role})`:''}</div></div>
              <div className="flex gap-1.5"><div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">X</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} type="number" value={selNode.x} onChange={e=>updNode(selNode.id,{x:Number(e.target.value)})}/></div>
              <div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">Y</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} type="number" value={selNode.y} onChange={e=>updNode(selNode.id,{y:Number(e.target.value)})}/></div></div>
              <div className="flex gap-1.5"><div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">W</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} type="number" value={selNode.width} onChange={e=>updNode(selNode.id,{width:Number(e.target.value)})}/></div>
              <div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">H</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} type="number" value={selNode.height} onChange={e=>updNode(selNode.id,{height:Number(e.target.value)})}/></div></div>
              {selNode.type==='text'&&<>
                <div><div className="text-[10px] text-wiki-text3 mb-1">内容</div><input className="w-full px-1.5 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.content||''} onChange={e=>updNode(selNode.id,{content:e.target.value})}/></div>
                <div className="flex gap-1.5"><div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">字号</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" type="number" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.fontSize||14} onChange={e=>updNode(selNode.id,{fontSize:Number(e.target.value)})}/></div>
                <div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">字重</div><select className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.fontWeight||400} onChange={e=>updNode(selNode.id,{fontWeight:Number(e.target.value)})}><option value={300}>300</option><option value={400}>400</option><option value={600}>600</option><option value={700}>700</option></select></div></div>
                <div><div className="text-[10px] text-wiki-text3 mb-1">字体</div><select className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.fontFamily||'system-ui'} onChange={e=>updNode(selNode.id,{fontFamily:e.target.value})}>{FONTS.map(f=><option key={f}>{f}</option>)}</select></div>
              </>}
              <div><div className="text-[10px] text-wiki-text3 mb-1">颜色</div><div className="flex flex-wrap gap-1">{PALETTE.slice(0,12).map(c=><button key={c} onClick={()=>updNode(selNode.id,{fill:c})} className="w-4 h-4 rounded-sm" style={{background:c,border:selNode.fill===c?'2px solid var(--wiki-text)':'1px solid var(--wiki-border)'}}/>)}</div></div>
              <div><div className="text-[10px] text-wiki-text3 mb-1">圆角</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" type="number" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.cornerRadius||0} onChange={e=>updNode(selNode.id,{cornerRadius:Number(e.target.value)})}/></div>
              {selNode.type==='frame'&&<>
                <div><div className="text-[10px] text-wiki-text3 mb-1">布局</div><select className="w-full px-1 py-0.5 rounded text-xs outline-none" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.layout||'none'} onChange={e=>updNode(selNode.id,{layout:e.target.value as any})}><option value="none">自由</option><option value="vertical">垂直</option><option value="horizontal">水平</option></select></div>
                <div className="flex gap-1.5"><div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">间距</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" type="number" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.gap||8} onChange={e=>updNode(selNode.id,{gap:Number(e.target.value)})}/></div>
                <div className="flex-1"><div className="text-[10px] text-wiki-text3 mb-1">内距</div><input className="w-full px-1 py-0.5 rounded text-xs outline-none" type="number" style={{background:'var(--wiki-surface2)',border:'1px solid var(--wiki-border)',color:'var(--wiki-text)'}} value={selNode.padding||16} onChange={e=>updNode(selNode.id,{padding:Number(e.target.value)})}/></div></div>
              </>}
              <button onClick={()=>delNode(selNode.id)} className="w-full py-1.5 rounded-lg text-xs font-medium" style={{background:'rgba(239,68,68,0.12)',color:'#ef4444'}}><TrashIcon size={11} className="inline"/> 删除</button>
            </div>:<div className="flex-1 flex items-center justify-center text-[10px] text-wiki-text3 p-3 text-center">选择元素编辑样式</div>}
          </div>):(
...
function DesignCreator({onClose}:{onClose:()=>void}) {
  ...
  const genFn=async()=>{if(!aiIn.trim())return;setChatMsgs(prev=>[...prev,{role:'user',content:aiIn}]);setGen(true);
    const sp=`你是资深UI/UX设计师。你必须严格只输出一个纯JSON数组，不要markdown代码块、不要解释文字。画布${vp}×768px。元素(DesignNode)必含type,x,y,width,height。type:frame(容器,可嵌套children,支持layout/gap/padding),rect,text(需content+fontSize),ellipse,image。颜色:#6366f1/#10b981/#f59e0b/#ef4444/#333/#fff/#f8f9fa/#e5e7eb。圆角8-16px。仅输出数组。生成完整页面15-30个节点。示例:[{"type":"frame","x":0,"y":0,"width":1024,"height":768,"fill":"#f8f9fa","children":[...]}]。`;
    try{const api=(window as any).electronAPI;if(!api?.chatSend)throw new Error('IPC不可用');const r=await api.chatSend({messages:[{role:'user',content:aiIn}],systemPrompt:sp,toolsEnabled:false});if(!r)throw new Error('无响应');if(r.error)throw new Error(r.error);let raw=(r.content||'').trim();if(!raw)throw new Error('AI返回空内容');let reply=raw.replace(/```[a-z]*\n?/gi,'').replace(/`/g,'').trim();const m=reply.match(/\[[\s\S]*\]/);if(!m)throw new Error('AI未返回JSON数组，开头: '+raw.substring(0,80));let parsed;try{parsed=JSON.parse(m[0])}catch(pe){throw new Error('JSON解析失败: '+pe.message)}if(!Array.isArray(parsed)||parsed.length===0)throw new Error('返回的不是有效数组');const fresh=parsed.map((el,i:any)=>({...el,id:'ai_'+Date.now()+'_'+i}));setNodes(aiIn.includes('修改')?[...nodes,...fresh]:fresh);setTitle(title||aiIn.substring(0,30));setChatMsgs(prev=>[...prev,{role:'assistant',content:'已生成'+fresh.length+'个节点'}]);toast.success('已生成'+fresh.length+'个节点');setAiIn('')}catch(e){const em=(e as any).message||String(e);setChatMsgs(prev=>[...prev,{role:'assistant',content:'失败: '+em}]);toast.error('生成失败: '+em)}finally{setGen(false)}};
